...
import (
	...
	profilehandler "github.com/ak-repo/ecommerce-gin/internals/customer/profile/handler"
	profilerepo "github.com/ak-repo/ecommerce-gin/internals/customer/profile/repository"
	profilesvc "github.com/ak-repo/ecommerce-gin/internals/customer/profile/service"
	reviewhander "github.com/ak-repo/ecommerce-gin/internals/customer/review/handler"
	reviewrepo "github.com/ak-repo/ecommerce-gin/internals/customer/review/repository"
	reviewsvc "github.com/ak-repo/ecommerce-gin/internals/customer/review/service"
...
func RegisterCustomerRoutes(r *gin.Engine, db *db.Database, cfg *config.Config) {
	...
	profileRepo := profilerepo.NewProfileRepo(db.DB)
	orderRepo := orderrepo.NewOrderRepo(db.DB)
	...
	reviewService := reviewsvc.NewReviewService(reviewRepo)
	profileService := profilesvc.NewProfileService(profileRepo, authRepo)
	...
	reviewHandler := reviewhander.NewReviewHandler(reviewService)
	...
	protected.Use(authmiddleware.AuthMiddleware(cfg), authmiddleware.RoleMiddleware("customer"))
	{
		// Auth Management
		...
		// Orders
		protected.GET("/orders", orderHandler.GetOrderByCustomerID)
